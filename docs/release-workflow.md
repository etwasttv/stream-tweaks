...
手動実行し、Fabric / NeoForge / Forge の JAR を Modrinth と GitHub Releases に公開する手順をまとめたものです。
...
- Actions タブ（または `gh workflow run`）から、対象の Minecraft バージョンブランチを選び、
  `channel`（`alpha` / `beta` / `release`）と `loaders`（`all` / `fabric` / `neoforge` / `forge`）を指定して実行します。
- タグは **Fabric / NeoForge / Forge 共通の1系列** です。ローダーごとにタグを分けることはしません。
...
| 選択 | 公開対象 | 用途 |
| --- | --- | --- |
| `all`（既定） | Fabric + NeoForge + Forge | 通常のリリース |
| `fabric` | Fabric のみ | 一部のローダーだけ修正を届けたいとき |
| `neoforge` | NeoForge のみ | 同上 |
| `forge` | Forge のみ | 同上 |

**`channel=release` では `loaders=all` のみ指定できます。** 安定版のタグは
`v<mod_version>+mc<mc>` 固定で、一部のローダーだけ公開すると残りを公開する2回目の実行で必ずタグが衝突するため、
`compute` ステップで即座に失敗させています。連番を採番する alpha / beta では単独ローダーの公開が可能です。
...
5. `build`: `loaders` に応じて、タグ push 前に対象ローダーをビルド
   ...
   - `forge`: `./gradlew clean :fabric:test :forge:build --stacktrace "-Pmod_version=<version>"`
6. `tag`: annotated tag を作成して push
7. `Publish Fabric to Modrinth`（`loaders` が `all` / `fabric` のとき）
8. `Publish NeoForge to Modrinth`（`loaders` が `all` / `neoforge` のとき）
9. `Publish Forge to Modrinth`（`loaders` が `all` / `forge` のとき）
10. `Publish to GitHub Release`: 選択したローダーの JAR をまとめて1つの Release に添付
11. `notify` ジョブ: 成功・失敗のどちらでも Discord に通知（`scripts/notify_discord.py`）
...
現状 `:fabric:test` にだけ存在するため、`neoforge` / `forge` 単独公開でも `:fabric:test` は実行します。
...
`^Loaders: ((fabric|neoforge|forge)(,(fabric|neoforge|forge))*)$` に一致しないタグ（`Loaders:` 行を持たない
既存の lightweight タグなど）は「不明」＝全ローダー公開済みとみなします。
...
`scripts/changelog_tool.py` を `--loaders` 付きで複数回呼び出し、4種類のノートを作ります。
...
| Forge 向け | `forge` | Forge の前回タグ | Modrinth の Forge version |
| まとめ | 選択したローダー（`all` なら `fabric,neoforge,forge`） | 選択ローダーのうち**最も古い**前回タグ | GitHub Release / Discord |

まとめノートで最も古い前回タグを使うのは、一部のローダーだけ先に公開していた場合でも変更点を
取りこぼさないようにするためです。

## CHANGELOG のローダータグ記法

`CHANGELOG.md` の箇条書き先頭に `[fabric]` / `[neoforge]` / `[forge]` / `[fabric,neoforge,forge]` を
付けると、リリースノート生成時にローダーで絞り込まれます。
...
- [fabric,neoforge,forge] 全ローダーに関係するが、記述を分けたい場合
```
...
- **本文が完全に同じ内容なら、`[fabric,neoforge,forge]` のように1件にまとめてください。** 同じ内容を
  `[fabric]` と `[neoforge]` のように分けて書くと、まとめノートに複数行として出てしまいます。
...
- **`channel=release` で即失敗する**: `loaders=all` になっているか、`CHANGELOG.md` に
  `## [<mod_version>]` セクションがあるかを確認します。
- **リリースノートが「変更点はありません。」になる**: 選択したローダー向けの箇条書きが
  `[Unreleased]` に無い（前回タグ以降に差分が無い）状態です。
- **成果物が見つからない**: JAR は root の `build/libs` ではなく `fabric/build/libs` /
  `neoforge/build/libs` / `forge/build/libs` に出力されます。
