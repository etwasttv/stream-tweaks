...
/**
 * Twitchチャンネル接続用の画面（認証状態・チャンネル接続/切断）。
 *
 ...
 * Paneにまとめている。"Config" / "Back" ボタンだけは {@link HeaderAndFooterLayout} のフッター領域に置き、
 ...
 * および対応するテストは本変更で削除した）。
 */
public class TwitchConnectionScreen extends Screen {
    ...
    private static final int CONFIG_BUTTON_WIDTH = 100;
    ...
    /** 画面全体のヘッダー(タイトル)／コンテンツ／フッター(Config/Backボタン)を管理するレイアウト。 */
    private HeaderAndFooterLayout layout;

    /**
     * "Twitch Connection Status:" から "Connected Channels:" のチャンネル一覧までをまとめてラップする
     * スクロール可能領域。Config/Backボタンはこの外（{@link #layout}のフッター）に置かれるため、
     * スクロールの影響を受けない。
     */
    private ScrollableLayout scrollArea;

    public TwitchConnectionScreen(Screen parent) {
    ...
    protected void init() {
        ...
        Button configBtn = Button.builder(Component.literal("Config"), button -> openConfigScreen())
                .width(CONFIG_BUTTON_WIDTH)
                .build();
        configBtn.active = !busy;
    ...
    /**
     * Config/Backボタン(フッター)を除いたコンテンツ領域に確保できる最大高さ。
     *
    ...
    private void openConfigScreen() {
        ...
        this.minecraft.gui.setScreen(new StreamTweaksConfigScreen(this));
    }
